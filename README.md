...
Временная сборка и публикация в `ghcr.io` только для `amd64`:
...
Текущий [Dockerfile](/home/evmenenko/Documents/gbunker/tproxy/Dockerfile) сейчас используется в режиме `linux/amd64`. Возврат `arm64` можно сделать позже без смены структуры образа.
